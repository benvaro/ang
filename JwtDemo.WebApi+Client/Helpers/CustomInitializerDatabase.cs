    ...
    public class CustomInitializerDatabase
    {
            ...
            IConfiguration config)
        {
            using (var scope = service.GetRequiredService<IServiceScopeFactory>().CreateScope())
            {
                ...
                AddUsers(manager, managerRole);
            }
        }

        private static void AddUsers(UserManager<User> manager, RoleManager<IdentityRole> managerRole)
        {
            var roleAdmin = "Admin";

            if (managerRole.FindByNameAsync(roleAdmin).Result == null)
            {
